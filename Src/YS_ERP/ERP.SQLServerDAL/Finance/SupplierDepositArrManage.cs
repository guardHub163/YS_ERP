...
{
    public class SupplierDepositArrManage : ISupplierDepositArr
    {
        #region  Method
        ...
        public string GetMaxID()
        {
            ...
            sb.Append(" select ISNULL(max(SLIP_NUMBER),0) FROM BLL_SUPPLIER_DEPOSIT_ARR ");
        ...
        public bool Delete(string slipNumber, string lineNumber)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("UPDATE BLL_SUPPLIER_DEPOSIT_ARR_LINE ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from bll_supplier_deposit_arr_view");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetList(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.* from bll_supplier_deposit_arr_view T");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" SELECT isnull(sum(ARR_AMOUNT),0) as ARR_AMOUNT from BLL_SUPPLIER_DEPOSIT_ARR_LINE ");
            strSql.AppendFormat(" WHERE STATUS_FLAG <> {0} ", CConstant.DELETE);
            strSql.Append(" AND PURCHASE_ORDER_SLIP_NUMBER=@PURCHASE_ORDER_SLIP_NUMBER");
            SqlParameter[] parameters = {
					new SqlParameter("@PURCHASE_ORDER_SLIP_NUMBER", SqlDbType.VarChar,50)
                                        };
