    ...
    public class CompanyManage : ICompany
    {
        ...
        public bool Add(CZZD.ERP.Model.BaseCompanyTable model)
        {
            ...
            else
            {
                ...
                strSql.Append("CODE,NAME,NAME_SHORT,NAME_ENGLISH,ZIP_CODE,ADDRESS_FIRST,ADDRESS_MIDDLE,ADDRESS_LAST,PHONE_NUMBER,FAX_NUMBER,EMAIL,URL,MEMO,STATUS_FLAG,CREATE_USER,CREATE_DATE_TIME,LAST_UPDATE_USER,LAST_UPDATE_TIME,LOGO,LOGO_TITLE)");
                strSql.Append(" values (");
                strSql.Append("@CODE,@NAME,@NAME_SHORT,@NAME_ENGLISH,@ZIP_CODE,@ADDRESS_FIRST,@ADDRESS_MIDDLE,@ADDRESS_LAST,@PHONE_NUMBER,@FAX_NUMBER,@EMAIL,@URL,@MEMO,@STATUS_FLAG,@CREATE_USER,GETDATE(),@LAST_UPDATE_USER,GETDATE(),@LOGO,@LOGO_TITLE)");
                SqlParameter[] parameters = {
					new SqlParameter("@CODE", SqlDbType.VarChar,20),
					new SqlParameter("@NAME", SqlDbType.NVarChar,100),
					new SqlParameter("@NAME_SHORT", SqlDbType.NVarChar,50),
					...
					new SqlParameter("@LOGO_TITLE", SqlDbType.Image)
                                            };
                parameters[0].Value = model.CODE;
                parameters[1].Value = model.NAME;
                parameters[2].Value = model.NAME_SHORT;
                ...
                parameters[11].Value = model.URL;
                parameters[12].Value = model.MEMO;
                parameters[13].Value = CConstant.INIT;
                parameters[14].Value = model.CREATE_USER;
                parameters[15].Value = model.LAST_UPDATE_USER;
                parameters[16].Value = model.LOGO;
                parameters[17].Value = model.COMPANY_PICTURE;
                rows = DbHelperSQL.ExecuteSql(strSql.ToString(), parameters);
                #endregion
            }
            if (rows > 0)
        ...
        public bool Update(CZZD.ERP.Model.BaseCompanyTable model)
        {
            ...
            strSql.Append("LOGO_TITLE=@LOGO_TITLE");
            strSql.Append(" where CODE=@CODE ");
            SqlParameter[] parameters = {
					new SqlParameter("@CODE", SqlDbType.VarChar,20),
					new SqlParameter("@NAME", SqlDbType.NVarChar,100),
            ...
            parameters[15].Value = model.LOGO;
            parameters[16].Value = model.COMPANY_PICTURE;
        ...
        public bool Delete(string CODE)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("update BASE_COMPANY  set STATUS_FLAG = {0}", CConstant.DELETE);
            strSql.Append(" where CODE=@CODE ");
            SqlParameter[] parameters = {
					new SqlParameter("@CODE", SqlDbType.VarChar,50)};
            parameters[0].Value = CODE;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 CODE,NAME,NAME_SHORT,NAME_ENGLISH,ZIP_CODE,ADDRESS_FIRST,ADDRESS_MIDDLE,ADDRESS_LAST,PHONE_NUMBER,FAX_NUMBER,EMAIL,URL,MEMO,STATUS_FLAG,CREATE_USER,CREATE_DATE_TIME,LAST_UPDATE_USER,LAST_UPDATE_TIME,LOGO,LOGO_TITLE from BASE_COMPANY ");
            strSql.Append(" where CODE=@CODE ");
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    model.LOGO = ((Byte[])ds.Tables[0].Rows[0]["LOGO"]);
                }
                if (ds.Tables[0].Rows[0]["LOGO_TITLE"].ToString() != "")
                {
                    model.COMPANY_PICTURE = ((Byte[])ds.Tables[0].Rows[0]["LOGO_TITLE"]);
                }
