    ...
    public class SupplierManage:ISupplier
    {
        ...
        public bool Add(CZZD.ERP.Model.BaseSupplierTable model)
        {
            ...
            if (Exists(model.CODE))
            {
                ...
                strSql.Append("PHONE_NUMBER_LAST=@PHONE_NUMBER_LAST,");
                strSql.Append("FAX_NUMBER_LAST=@FAX_NUMBER_LAST");
                strSql.Append(" where CODE=@CODE ");
                SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@PHONE_NUMBER_LAST", SqlDbType.VarChar,20),
					new SqlParameter("@FAX_NUMBER_LAST", SqlDbType.VarChar,20)};
                ...
                parameters[22].Value = model.PHONE_NUMBER_LAST;
                parameters[23].Value = model.FAX_NUMBER_LAST;
            ...
            else
            {
                ...
                strSql.Append("CODE,NAME,NAME_SHORT,NAME_ENGLISH,ZIP_CODE,ADDRESS_FIRST,ADDRESS_MIDDLE,ADDRESS_LAST,PHONE_NUMBER,FAX_NUMBER,MOBIL_NUMBER,CONTACT_NAME,EMAIL,URL,MEMO,TYPE,CLAIM_FLAG,CLAIM_CODE,CURRENCE_CODE,STATUS_FLAG,CREATE_USER,CREATE_DATE_TIME,LAST_UPDATE_USER,LAST_UPDATE_TIME,PHONE_NUMBER_LAST,FAX_NUMBER_LAST)");
                strSql.Append(" values (");
                strSql.Append("@CODE,@NAME,@NAME_SHORT,@NAME_ENGLISH,@ZIP_CODE,@ADDRESS_FIRST,@ADDRESS_MIDDLE,@ADDRESS_LAST,@PHONE_NUMBER,@FAX_NUMBER,@MOBIL_NUMBER,@CONTACT_NAME,@EMAIL,@URL,@MEMO,@TYPE,@CLAIM_FLAG,@CLAIM_CODE,@CURRENCE_CODE,@STATUS_FLAG,@CREATE_USER,GETDATE(),@LAST_UPDATE_USER,GETDATE(),@PHONE_NUMBER_LAST,@FAX_NUMBER_LAST)");
                SqlParameter[] parameters = {
					new SqlParameter("@CODE", SqlDbType.VarChar,20),
					new SqlParameter("@NAME", SqlDbType.NVarChar,100),
					new SqlParameter("@NAME_SHORT", SqlDbType.NVarChar,50),
        ...
        public bool Update(CZZD.ERP.Model.BaseSupplierTable model)
        {
            ...
            parameters[21].Value = model.PHONE_NUMBER_LAST;
            parameters[22].Value = model.FAX_NUMBER_LAST;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("update BASE_SUPPLIER  set STATUS_FLAG = {0}", CConstant.DELETE);
            strSql.Append(" where CODE=@CODE ");
            SqlParameter[] parameters = {
                      new SqlParameter("@CODE", SqlDbType.VarChar,50)};
            parameters[0].Value = CODE;
        ...
        public CZZD.ERP.Model.BaseSupplierTable GetModel(string CODE)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                model.PHONE_NUMBER_LAST = ds.Tables[0].Rows[0]["PHONE_NUMBER_LAST"].ToString();
                model.FAX_NUMBER_LAST = ds.Tables[0].Rows[0]["FAX_NUMBER_LAST"].ToString();
                return model;
            }
            else
            {
